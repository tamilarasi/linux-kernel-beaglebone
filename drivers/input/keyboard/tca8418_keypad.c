			      ...
			      int reg, u8 val)
{
	...
	printk("tca8418 write byte =%d",error);
...
static void tca8418_read_keypad(struct tca8418_keypad *keypad_data)
{
	...
	input_sync(keypad_data->input);
     //   printk("tca8418 keyscan found\n");
}
#if 0
static void tca8418_scan(struct work_struct *work)
{
struct tca8418_keypad *keypad_data =
		container_of(work, struct tca8418_keypad, dwork.work);
int error,reg;
...
exit:
	...
	if (error)
        ...
        printk("tca8418 keyscan\n");

schedule_delayed_work(&keypad_data->dwork, msecs_to_jiffies(1000));
}
...
{
	int reg, error;
        printk("tca8418 keyscan in configure\n");
					  ...
					  const struct i2c_device_id *id)
{
	...
	struct tca8418_keypad *keypad_data;
	struct input_dev *input;
	int error, row_shift, max_keys,status;
        printk("tca8418 keyscan key probe\n");
	...
	if (error < 0)
		goto fail1;  //This module is commented by tamilarasi
	...
	if (pdata->irq_is_gpio)
		{
		status=gpio_request(client->irq, "BONE_LCD_BL");
		if (status < 0)
		printk("Failed to request gpio for keypad int");
	...
	//INIT_DELAYED_WORK(&keypad_data->dwork, tca8418_scan);
	//schedule_delayed_work(&keypad_data->dwork, msecs_to_jiffies(1000));
	return 0;
